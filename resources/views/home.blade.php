...
@foreach ($find_containers as $index => $container)
    @php
        ...
        $last_totalSale += $totalSale;
        $last_totalCost += $totalCost;

        $profit_loss = $totalSale - $totalCost;

        if ($profit_loss > 0) {
            $last_totalProfit += $profit_loss;
        } elseif ($profit_loss < 0) {
            $last_totalLoss += abs($profit_loss);
        }
    @endphp
@endforeach
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0">
                        <div class="card-body p-0 d-flex align-items-center shadow-sm">
                            ...
                            <div>
                                <div class="text-value text-primary">{{ format_currency(round($last_totalCost)) }}</div>
                                <div class="text-muted text-uppercase font-weight-bold small">Total Cost</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0">
                        <div class="card-body p-0 d-flex align-items-center shadow-sm">
                            ...
                            <div>
                                <div class="text-value text-warning">{{ format_currency(round($last_totalSale)) }}</div>
                ...
                <div class="card modern-card border-0 h-100">
                    ...
                    <div class="card-body">
                        @php
                            ...
                            $customDonePercent =
                                $totalContainers > 0 ? round(($customDone / $totalContainers) * 100) : 0;

                            // Get yesterday's counts for comparison
$yesterdayPending = DB::table('container')
    ->where('status', 0)
    ->whereDate('created_at', today()->subDay())
    ->count();
$yesterdayShipped = DB::table('container')
    ->where('status', 1)
    ->whereDate('created_at', today()->subDay())
    ->count();
$yesterdayArrived = DB::table('container')
    ->where('status', 2)
    ->whereDate('created_at', today()->subDay())
    ->count();
$yesterdayCustomDone = DB::table('container')
    ->where('status', 3)
    ->whereDate('created_at', today()->subDay())
                                ->count();
                                ...
                                <div class="modern-status-card p-3 rounded position-relative h-100">
                                    ...
                                    <div class="progress modern-progress mt-2">
                                        <div class="progress-bar bg-blue" role="progressbar"
                                            style="width: {{ $pendingPercent }}%" aria-valuenow="{{ $pendingPercent }}"
                                ...
                                <div class="modern-status-card p-3 rounded position-relative h-100">
                                    ...
                                    <div class="progress modern-progress mt-2">
                                        <div class="progress-bar bg-green" role="progressbar"
                                            style="width: {{ $shippedPercent }}%" aria-valuenow="{{ $shippedPercent }}"
                                            aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="modern-status-card p-3 rounded position-relative h-100">
                                    ...
                                    <div class="progress modern-progress mt-2">
                                        <div class="progress-bar bg-orange" role="progressbar"
                                            style="width: {{ $arrivedPercent }}%" aria-valuenow="{{ $arrivedPercent }}"
                                            aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="modern-status-card p-3 rounded position-relative h-100">
                                    ...
                                    <div class="progress modern-progress mt-2">
                                        <div class="progress-bar bg-purple" role="progressbar"
                                            style="width: {{ $customDonePercent }}%"
                                            aria-valuenow="{{ $customDonePercent }}" aria-valuemin="0"
                        ...
                        <div class="mt-4 pt-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="text-dark-600 small">Total Containers: <span
                                        class="fw-semibold text-dark">{{ $totalContainers }}</span></div>
                                <div class="text-dark-600 small">Updated: <span
                                        class="fw-semibold text-dark">{{ now()->format('M j, Y g:i A') }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    @forelse ($containers as $container)
                                        <tr class="align-middle animate__animated animate__fadeInUp"
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($container->shipping_date)->format('d M Y') }}
                                            </td>
                                            <td>
        ...
        <div class="col-lg-12">
            <div class="card border-0 shadow-sm h-30">
                <div class="card-header bg-gradient-info text-white d-flex justify-content-between align-items-center">
                    ...
                    @php
                        ...
                        foreach ($banks as $bank) {
                            $transactions = DB::table('transactions')
                                ...
                                ->select(DB::raw('SUM(in_amount) as total_in'), DB::raw('SUM(out_amount) as total_out'))
                                ->first();
                            ...
                            <tbody>
                                @forelse ($banks as $bank)
                                    <tr style="font-weight: 500;font-family: auto;font-size: large;">
                                        ...
                                        <td
                                            class="text-end fw-bold {{ $bank->opening_balance < 0 ? 'text-danger' : 'text-success' }}">
                                            {{ format_currency($bank->opening_balance) }}
                                        </td>
                                        <td
                                            class="text-end fw-bold {{ $bank->last_balance < 0 ? 'text-danger' : 'text-danger' }}">
                        ...
                        <div class="table-responsive">
                            <table id="cashflow-table" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    @forelse ($find_containers as $index => $container)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            ...
                                            @php
                                                ...
                                                $totalSale = \Modules\Sale\Entities\SaleDetails::where(
                                                    'lc_id',
                                                    $container->lc_id,
                                                )
                                                    ->where('container_id', $container->id)
