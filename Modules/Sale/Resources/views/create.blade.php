...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        Livewire.on('consoleLog', (data) => {
            console.warn("ddd", data); // Inspect payload structure in console

            // If data is an array, get the first element
            let payload = Array.isArray(data) ? data[0] : data;

            if (payload && payload.message && payload.message.includes("Insufficient stock")) {
                createBtn.disabled = true;
                createBtn.classList.add("btn-secondary");
                createBtn.classList.remove("btn-primary");

                // Add tooltip text
                createBtn.setAttribute("title", "You have Stock 0");
            } else {
                createBtn.disabled = false;
                createBtn.classList.add("btn-primary");
                createBtn.classList.remove("btn-secondary");

                // Remove tooltip when stock is available
